    ...
    return (
        <div className="fixed inset-0 w-full h-full pointer-events-none z-[-2] overflow-hidden bg-[#020617]">
            ...
            <motion.div 
                ...
                className="bg-[radial-gradient(circle_at_20%_30%,rgba(0,163,255,0.15)_0%,transparent_50%),radial-gradient(circle_at_80%_70%,rgba(0,209,255,0.1)_0%,transparent_50%)]"
            />

            {/* Distant Nebula Layer 2 */}
            <motion.div 
                style={{ y: nebulaY2 }}
                className="absolute top-0 left-0 w-full h-[200%] opacity-20"
                className="bg-[radial-gradient(circle_at_50%_50%,rgba(0,102,255,0.1)_0%,transparent_70%)]"
            />

            {/* Space Dust / Stars Layer */}
            <motion.div 
                style={{ y: dustY }}
                className="absolute top-0 left-0 w-full h-[300%] opacity-40"
            >
                {[...Array(50)].map((_, i) => (
                    <div 
                        key={i}
