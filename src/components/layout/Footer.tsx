...
export function Footer() {
    ...
    return (
        <footer className="bg-white border-t border-slate-200 overflow-hidden relative">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16 relative z-10">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8 lg:gap-12">
                    {/* Brand Col */}
                    <div className="sm:col-span-2 lg:col-span-2">
                        <Link href="/" onClick={(e) => { if (pathname === '/') { e.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' }); } }} className="flex items-center space-x-2 group mb-6">
                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-gvb-blue to-gvb-deep flex items-center justify-center shadow-lg transition-transform group-hover:scale-110">
                                <Logo className="w-6 h-6 text-white" glow={false} />
                            </div>
                            <span className="text-2xl font-bold tracking-tighter text-slate-800">GVB Tech</span>
                        </Link>
                        <p className="text-base text-slate-600 mb-6 font-medium max-w-sm">
                            Engineering tomorrow&apos;s solutions with proprietary trading, robust software, and strategic growth infrastructure.
                        </p>
                        <div className="flex space-x-5">
                            <a href="https://github.com/BalajiKoushik01" target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg bg-slate-50 text-slate-400 hover:text-gvb-blue hover:bg-slate-100 transition-all"><Github className="w-5 h-5" /></a>
                            <a href="https://www.linkedin.com/in/balaji-koushik-05222a316" target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg bg-slate-50 text-slate-400 hover:text-gvb-blue hover:bg-slate-100 transition-all"><Linkedin className="w-5 h-5" /></a>
                            <a href="https://www.instagram.com/balajikoushik01?igsh=NGNpem1tOG11ZHVp" target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg bg-slate-50 text-slate-400 hover:text-gvb-blue hover:bg-slate-100 transition-all"><Instagram className="w-5 h-5" /></a>
                        </div>
                    </div>
