...
export default function Dashboard() {
  const API_BASE = process.env.NEXT_PUBLIC_APEX_API_URL || 'https://balajikoushik-apex-backend.hf.space';
  ...
  const runCointegration = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/math/cointegration?${queryParams}`, {
        method: 'POST'
      });
      if (!res.ok) {
  ...
  const runVarModel = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/math/var?target_ticker=${varTarget.trim().toUpperCase()}&${queryParams}&lags=${varLags}`, {
        method: 'POST'
      });
      if (!res.ok) {
        const errDetail = await res.json();
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(`${API_BASE}/api/stocks/search?q=${encodeURIComponent(searchQuery.trim())}`);
        if (res.ok) {
  ...
  const fetchAlignedData = async (targetTicker: string, pipelinePreview?: any[]) => {
    try {
      ...
      const dataRes = await fetch(`${API_BASE}/api/data/preview?ticker=${targetTicker.toUpperCase()}`);
  ...
  const runPipeline = async (targetTicker?: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/pipeline/run?ticker=${t}&start_date=${startDate}`, { method: 'POST' });
      if (!res.ok) {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/pipeline/upload?ticker_name=${baseName}`, {
        method: 'POST',
        body: formData
      });
      if (!res.ok) {
  ...
  return (
    <div className="apex-page-wrapper">
                ...
                <div style={{ background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border-color)', borderRadius: '12px', padding: '1.2rem' }}>
                  ...
                  {fundamentals ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
                      ...
                      {/* Debt to Equity Leverage Ratio Gauge */}
                      <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', marginBottom: '6px' }}>
